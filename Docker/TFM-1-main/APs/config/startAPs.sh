...
# Marca todas las variables exportadas para que estén disponibles para los subprocesos posteriores
set -a
# Carga variables de entorno de wlan_config_aps
source /root/wlan_config_aps



# Reemplazamos las variables por sus valores correspondientes, generando archivos sin la extensión .tmp
...
# Reemplazamos los .tmp de cada directorio
...
# Creamos /root/logs, descartamos los mensajes de error
mkdir /root/logs/ 2> /dev/nil

# Ejecutamos cronAPs.sh en segundo plano y redirigimos stdout a /root/logs/cronAPs.log 
